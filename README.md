...
- [SchoolPalm Module Bridge](#schoolpalm-module-bridge)
  ...
  - [Usage](#usage)
    ...
    - [Relation Helpers](#relation-helpers)
      - [Define Relations in a Provider](#define-relations-in-a-provider)
      - [Build Registry and Load Relations](#build-registry-and-load-relations)
      - [Helper Quick Reference](#helper-quick-reference)
...
- **Developer guide (technical):** `docs/DEVELOPERS.md`
- **Ecosystem pitch (why it’s needed):** `docs/ECOSYSTEM_PITCH.md`
...
Use `SchoolPalm\ModuleBridge\Relations\Relation` to define relation configuration arrays in module `RelationProvider` classes, then resolve them through `RelationEngine`.

#### Define Relations in a Provider

```php
use SchoolPalm\ModuleBridge\Relations\Relation;
use SchoolPalm\ModuleBridge\Relations\RelationProvider;

final class StudentRelations implements RelationProvider
{
    public function relations(): array
    {
        return [
            'student' => Relation::belongsTo(
                \App\Contracts\StudentContract::class,
                'student_id',
                'id'
            ),

            'profile' => Relation::hasOne(
                \App\Contracts\ProfileContract::class,
                'student_id',
                'id'
            ),

            'results' => Relation::hasMany(
                \App\Contracts\ResultContract::class,
                'student_id',
                'id'
            ),

            'clubs' => Relation::belongsToMany(
                \App\Contracts\ClubContract::class,
                \App\Contracts\MembershipContract::class,
                'student_id', // pivot_parent_key
                'club_id',    // pivot_related_key
                'id',         // local_key
                'id'          // related foreign_key
            ),

            'avatar' => Relation::morphOne(
                \App\Contracts\MediaContract::class,
                'owner_id',
                'student',
                'id',
                'owner_type'
            ...
            'comments' => Relation::morphMany(
                \App\Contracts\CommentContract::class,
                'commentable_id',
                'student',
                'id',
                'commentable_type'
            ...
            'owner' => Relation::morphTo([
                'student' => \App\Contracts\StudentContract::class,
                'teacher' => \App\Contracts\TeacherContract::class,
...
#### Build Registry and Load Relations

```php
use SchoolPalm\ModuleBridge\Relations\RelationEngine;
use SchoolPalm\ModuleBridge\Relations\RelationRegistryBuilder;

$registry = app(RelationRegistryBuilder::class)->build($installedModules);

$engine = new RelationEngine($registry);
...
$items = $engine->load($items, [
    ...
    'results',
    'comments',
    'student.address', // nested relation
]);
```

#### Helper Quick Reference

- `Relation::belongsTo(string $contract, string $localKey, string $foreignKey = 'id')`
- `Relation::hasOne(string $contract, string $foreignKey, string $localKey = 'id')`
- `Relation::hasMany(string $contract, string $foreignKey, string $localKey = 'id')`
- `Relation::belongsToMany(string $contract, string $pivotContract, string $pivotParentKey, string $pivotRelatedKey, string $localKey = 'id', string $foreignKey = 'id')`
- `Relation::morphOne(string $contract, string $foreignKey, string $morphType, string $localKey = 'id', string $morphTypeKey = 'morph_type')`
- `Relation::morphMany(string $contract, string $foreignKey, string $morphType, string $localKey = 'id', string $morphTypeKey = 'morph_type')`
- `Relation::morphTo(array $morphMap, string $typeField = 'morph_type', string $idField = 'morph_id', string $foreignKey = 'id')`
